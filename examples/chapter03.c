...
void demonstrate_bitwise() {
    ...
    unsigned char reg = 48; // Thanh ghi giả lập ban đầu bằng 0 (0011 0000)
    printf("Thanh ghi (reg) ban dau:       "); print_binary(reg); printf("\n");

    // Ứng dụng: Set bit (Bật bit) bằng OR (|) và LEFT SHIFT (<<)
    // Ví dụ: Bật bit ở vị trí số 3 (tính từ 0 từ phải qua trái)
    reg = reg | (1 << 3); //(1 << 3) = 0000 1000
...
void demonstrate_datatypes() {
    ...
    // printf("%-10s | %lu byte    | %-14d -> %-13d | 0 -> 2^8-1",
